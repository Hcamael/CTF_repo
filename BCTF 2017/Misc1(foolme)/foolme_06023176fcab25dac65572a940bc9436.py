...
def tofile(data):
    try:
        ...
        return ""
    filename="/backup/"+token+"."+str(time.time())+".jpg"
    fd=open(filename,"wb")
    fd.write(data)
    fd.close()
    filename="/home/ctf/images/"+token+"."+str(time.time())+".jpg"
    fd=open(filename,"wb")
    fd.write(data)
    fd.close()
    return filename
...
def main():
    ...
    if input_image_class!=std_image_class:
        print("[*]I think this is "+input_image_classname+".")
        print("[*]What? This is "+std_image_classname+"?")
        print("[*]You fooled me? impossible!")
        print(getflag())
        sys.stdout.flush()
        return
    else:
        print("[*]I think this is "+input_image_classname+", right?")
        print("[*]HAHA! You never fool me!")
        sys.stdout.flush()
        return
